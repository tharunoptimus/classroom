...
$(document).ready(async () => {
    ...
    if(isOwnerForAssignments) {
        ...
        await $.get(`/api/assignments/${classObject._id}/all`, (data) => {
            if(data.assignments.length < 0) return
            let html = renderAssignment(data.assignments, true)
        ...
        await $.get(`/api/assignments/${classObject._id}/all`, (data) => {
            ...
            data.assignments.forEach(assignment => {
                if(!userLoggedIn.assignments.includes(assignment._id)) {
                    arrayToShow.push(assignment);
                }
            })
            
            let html = renderAssignment(arrayToShow, false)
            $(".assignments").html(html)
            let button = `<button class="goodAjaxButton loadPreviousAssignments">Load Completed Assignments</button>`;
...
function renderTipsForOwner () {
    let html = `<div class="tipsBlock">
        <div class="tipsBlockHeader">
            <p class="standardText">Assign work to your class here</p>
        </div>
        <div class="tipsBlockBody">
            ...
            </div><span>Create assignments and reward marks!</span>
        </div>
    </div>`
...
function renderTipsForStudent () {
    let html = `<div class="tipsBlock">
        <div class="tipsBlockHeader">
            <p class="standardText">Take care of your assignments</p>
        </div>
        <div class="tipsBlockBody">
            <div class="tipsBlockSubBody">
...
function renderCreateAssignmentButton () {
    return `<button class="goodAjaxButton" style="outline:none;" id="createAnAssignment" data-toggle="modal" data-target="#createAssignmentModal">Create an Assignment</button>`
}

function renderAssignment (data, isOwner) {
    let html = ""
    data.reverse().forEach(assignment => {
        ...
        <div class="assignmentTitle" data-id="${assignment._id}">
            <p class="assignmentTitlePara">${assignment.name}</p><span class="assignmentTotalMarks">${assignment.assignedMarks}</span>
        </div>
...
$(document).on('click', '#confirmCreateAssignment', function () {
    ...
    $.post('/api/assignments/create', data, (data, status, xhr) => {
...
$(document).on('click', '.seeMoreDetailsButton', function () {
    let assignmentId = $(this).parent().parent().parent().find('.assignmentTitle').attr('data-id')
    let url = `/assignment/${classObject._id}/${assignmentId}`
...
$(document).on('click', '.loadPreviousAssignments', async () => {
    await $.get(`/api/assignments/${classObject._id}/submitted`, (data, xhr, status) => {
